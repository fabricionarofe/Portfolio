...
class UrlTest(unittest.TestCase):
    def test_address_in_network(self):
        self.assertTrue(_is_address_in_network("[IP_REMOVED]", "127.0.0.0/8"))
...
class IsNoProxyHostTest(unittest.TestCase):
    ...
    def test_ip_address(self):
        self.assertTrue(_is_no_proxy_host("[IP_REMOVED]", ["[IP_REMOVED]"]))
        self.assertFalse(_is_no_proxy_host("127.0.0.2", ["[IP_REMOVED]"]))
        self.assertTrue(
            _is_no_proxy_host("[IP_REMOVED]", ["other.websocket.org", "[IP_REMOVED]"])
        )
        self.assertFalse(
            _is_no_proxy_host("127.0.0.2", ["other.websocket.org", "[IP_REMOVED]"])
        )
        os.environ["no_proxy"] = "[IP_REMOVED]"
        self.assertTrue(_is_no_proxy_host("[IP_REMOVED]", None))
        self.assertFalse(_is_no_proxy_host("127.0.0.2", None))
        os.environ["no_proxy"] = "other.websocket.org, [IP_REMOVED]"
        self.assertTrue(_is_no_proxy_host("[IP_REMOVED]", None))
        self.assertFalse(_is_no_proxy_host("127.0.0.2", None))

    def test_ip_address_in_range(self):
        self.assertTrue(_is_no_proxy_host("[IP_REMOVED]", ["127.0.0.0/8"]))
...
class ProxyInfoTest(unittest.TestCase):
    ...
    def test_proxy_from_args(self):
        self.assertRaises(
            ...
            False,
            proxy_host="[HOST_REMOVED]",
        )
        self.assertEqual(
            get_proxy_info(
                "echo.websocket.events", False, proxy_host="[HOST_REMOVED]", proxy_port=3128
            ),
            ("[HOST_REMOVED]", 3128, None),
        )
        self.assertEqual(
            get_proxy_info(
                "echo.websocket.events", True, proxy_host="[HOST_REMOVED]", proxy_port=3128
            ),
            ("[HOST_REMOVED]", 3128, None),
        )
    ...
    def test_proxy_from_env(self):
        os.environ["http_proxy"] = "http://[HOST_REMOVED]/"
        self.assertEqual(
            get_proxy_info("echo.websocket.events", False), ("[HOST_REMOVED]", None, None)
        )
        os.environ["http_proxy"] = "http://[HOST_REMOVED]:3128/"
        self.assertEqual(
            get_proxy_info("echo.websocket.events", False), ("[HOST_REMOVED]", 3128, None)
        )
        ...
        os.environ["http_proxy"] = "http://a:b@[HOST_REMOVED]/"
        self.assertEqual(
            get_proxy_info("echo.websocket.events", False),
            ("[HOST_REMOVED]", None, ("a", "b")),
        )
        os.environ["http_proxy"] = "http://a:b@[HOST_REMOVED]:3128/"
        self.assertEqual(
            get_proxy_info("echo.websocket.events", False),
            ("[HOST_REMOVED]", 3128, ("a", "b")),
        )
        ...
        os.environ["http_proxy"] = (
            "http://john%40example.com:P%40SSWORD@[HOST_REMOVED]:3128/"
        )
        os.environ["https_proxy"] = (
        ...
        self.assertEqual(get_proxy_info("[IP_REMOVED]", False), (None, 0, None))
