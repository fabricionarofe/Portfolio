...
def test_format_host_port() -> None:
    assert format_host_port("[IP_REMOVED]", 80) == "[IP_REMOVED]:80"
    assert format_host_port(b"[IP_REMOVED]", 80) == "[IP_REMOVED]:80"
...
# Make sure we can connect to [HOST_REMOVED] using real kernel sockets
async def test_open_tcp_stream_real_socket_smoketest() -> None:
    listen_sock = trio.socket.socket()
    await listen_sock.bind(("[IP_REMOVED]", 0))
    _, listen_port = listen_sock.getsockname()
    listen_sock.listen(1)
    client_stream = await open_tcp_stream("[IP_REMOVED]", listen_port)
...
async def test_open_tcp_stream_input_validation() -> None:
    ...
    with pytest.raises(TypeError):
        await open_tcp_stream("[IP_REMOVED]", b"80")  # type: ignore[arg-type]
...
async def test_local_address_real() -> None:
    with trio.socket.socket() as listener:
        ...
        # certain we have is [IP_REMOVED], so we can't really test local_address=
        # properly -- passing local_address=[IP_REMOVED] is indistinguishable
        # from not passing local_address= at all. But, we can still do a smoke
        # test to make sure the local_address= code doesn't crash.
        local_address = "127.0.0.2" if can_bind_127_0_0_2() else "[IP_REMOVED]"
