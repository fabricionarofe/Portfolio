...
async def test_getaddrinfo(monkeygai: MonkeypatchedGAI) -> None:
    ...
    with assert_checkpoints():
        res = await tsocket.getaddrinfo("[IP_REMOVED]", "12345", type=tsocket.SOCK_STREAM)

    check(
        res,
        [
            (
                tsocket.AF_INET,  # [IP_REMOVED] is ipv4
                tsocket.SOCK_STREAM,
                tsocket.IPPROTO_TCP,
                "",
                ("[IP_REMOVED]", 12345),
...
async def test_getnameinfo() -> None:
    ...
    with assert_checkpoints():
        got = await tsocket.getnameinfo(("[IP_REMOVED]", 1234), ni_numeric)
    assert got == ("[IP_REMOVED]", "1234")
    ...
    with assert_checkpoints():
        with pytest.raises(tsocket.gaierror):
            await tsocket.getnameinfo(("[HOST_REMOVED]", 80), 0)

    # Blocking call to get expected values:
    host, service = stdlib_socket.getnameinfo(("[IP_REMOVED]", 80), 0)

    # Some working calls:
    got = await tsocket.getnameinfo(("[IP_REMOVED]", 80), 0)
    assert got == (host, service)

    got = await tsocket.getnameinfo(("[IP_REMOVED]", 80), tsocket.NI_NUMERICHOST)
    assert got == ("[IP_REMOVED]", service)

    got = await tsocket.getnameinfo(("[IP_REMOVED]", 80), tsocket.NI_NUMERICSERV)
    assert got == (host, "80")
...
async def test_SocketType_resolve(socket_type: AddressFamily, addrs: Addresses) -> None:
    ...
    with tsocket.socket(family=socket_type) as sock:
        ...
        for null in [None, ""]:
            ...
            assert not isinstance(got, (str, bytes))
            assert_eq(got, (addrs.[HOST_REMOVED], 80))
...
async def test_SocketType_unresolved_names() -> None:
    with tsocket.socket() as sock:
        await sock.bind(("[HOST_REMOVED]", 0))
        assert sock.getsockname()[0] == "[IP_REMOVED]"
        sock.listen(10)

        with tsocket.socket() as sock2:
            await sock2.connect(("[HOST_REMOVED]", sock.getsockname()[1]))
...
async def test_SocketType_connect_paths() -> None:
    with tsocket.socket() as sock:
        with pytest.raises(
            ...
            # Should be a tuple
            await sock.connect("[HOST_REMOVED]")
    ...
    with tsocket.socket() as sock:
        with pytest.raises(
            ...
            await sock.connect(("[IP_REMOVED]", 2))
...
async def test_send_recv_variants() -> None:
    ...
    with a, b:
        await a.bind(("[IP_REMOVED]", 0))
        await b.bind(("[IP_REMOVED]", 0))

        targets = [b.getsockname(), ("[HOST_REMOVED]", b.getsockname()[1])]
...
async def test_custom_hostname_resolver(monkeygai: MonkeypatchedGAI) -> None:
    ...
    for vals in [
        ...
        assert await tsocket.getaddrinfo("[HOST_REMOVED]", "foo", *vals) == (
            "custom_gai",
            b"[HOST_REMOVED]",
            "foo",
            *vals,
