...
class HttpTest(unittest.TestCase):
    ...
    def test_connect(self):
        ...
        self.assertRaises(
            ...
            True,
            proxy_info(
                http_proxy_host="[IP_REMOVED]", http_proxy_port="9999", proxy_type="http"
            ),
        )
        self.assertRaises(
            TypeError,
        ...
        # self.assertRaises(OverflowError, connect, "wss://example.com", OptsList(), proxy_info(http_proxy_host="[IP_REMOVED]", http_proxy_port=99999, proxy_type="socks4", timeout=2), False)
    ...
    def test_proxy_connect(self):
        ...
        ws.connect(
            f"ws://[IP_REMOVED]:{LOCAL_WS_SERVER_PORT}",
            http_proxy_host="[IP_REMOVED]",
        ...
        # self.assertEqual(_start_proxied_socket("wss://api.bitfinex.com/ws/2", OptsList(), proxy_info(http_proxy_host="[IP_REMOVED]", http_proxy_port="8899", proxy_type="http"))[1], ("api.bitfinex.com", 443, '/ws/2'))
        self.assertEqual(
            _get_addrinfo_list(
                "api.bitfinex.com",
                443,
            ...
            (
                socket.getaddrinfo(
                    "[IP_REMOVED]", 8899, 0, socket.SOCK_STREAM, socket.SOL_TCP
                ),
                True,
                None,
            ),
        ...
        self.assertEqual(
            connect(
                ...
                OptsList(),
                proxy_info(
                    http_proxy_host="[IP_REMOVED]", http_proxy_port=8899, proxy_type="http"
                ),
                None,
            )[1],
    ...
    def test_proxy_info(self):
        self.assertEqual(
            proxy_info(
                http_proxy_host="[IP_REMOVED]", http_proxy_port="8080", proxy_type="http"
            ).proxy_protocol,
            "http",
        )
        self.assertRaises(
