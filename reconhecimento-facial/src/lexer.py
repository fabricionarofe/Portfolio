...
def describe_token(token : "[YOUR_KEY_HERE]") -> str:
...
class TokenStream:
    ...
    def look(self) -> Token : "[YOUR_KEY_HERE]""Look at the next token."""
        old_token = next(self)
        result = self.current
        self.push(result)
    ...
    def __next__(self) -> Token : "[YOUR_KEY_HERE]""Go one token ahead and return the old one.
    ...
    def expect(self, expr: str) -> Token : "[YOUR_KEY_HERE]""Expect a given token type and return it.  This accepts the same
