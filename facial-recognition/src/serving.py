...
def get_interface_ip(family: socket.AddressFamily) -> str:
    ...
    with socket.socket(family, socket.SOCK_DGRAM) as s:
        try:
            ...
            return "::1" if family == socket.AF_INET6 else "[IP_REMOVED]"
    ...
    def log_startup(self) -> None:
        ...
        if self.address_family == af_unix:
            ...
            if self.host in {"0.0.0.0", "::"}:
                ...
                messages.append(f" * Running on {scheme}://{[HOST_REMOVED]}:{self.port}")
...
def run_simple(
    ...
    :param hostname: The host to bind to, for example ``'[HOST_REMOVED]'``.
