...
async def test_open_tcp_listeners_ipv6_v6only() -> None:
    ...
    async with ipv6_listener:
        ...
        with pytest.raises(
            ...
            await open_tcp_stream("[IP_REMOVED]", port)


async def test_open_tcp_listeners_rebind() -> None:
    (l1,) = await open_tcp_listeners(0, host="[IP_REMOVED]")
    ...
    await l1.aclose()
    (l2,) = await open_tcp_listeners(sockaddr1[1], host="[IP_REMOVED]")
